...
new_model = torch.load("cnn_model")
counter = 0
...
def threaded_image_handling(cv2_img):
	global turn, recognition_running, counter
	#cv2.imwrite('camera_img.jpeg', cv2_img)	## save image
	#print("Came here *************")
	#stop, right, left = get_patches(cv2_img)
	#out = [stop[0], right[0], left[0]]
	#print(out)
	#turns = ["stop", "right", "left"]
	#turn = turns[out.index(max(out))]
	#print("*************"+turn)	
	
	print("Processing Image data")
	print("writing data")
	cv2.imwrite('./test/test/camera_img.jpeg', cv2_img)	## save image
	print("Wrote data")
	##call your recognition model here and set the value of turn
	data_transforms1 = { 'test': transforms.Compose([
	...
	image_datasets1 = {x: datasets.ImageFolder('test',
											  data_transforms1[x])
					  for x in ['test']}
	dataloaders1 = {x: torch.utils.data.DataLoader(image_datasets1[x], batch_size=1,
                                             shuffle=True)
              for x in ['test']}
	for data in dataloaders1['test']:
	# get the inputs
	  inputs1 = data
	print("Step3")
	predsArr = []
	outputs1 = new_model(Variable(inputs1[0]))
	_, preds1 = torch.max(outputs1.data, 1)
	map = { 0: "left", 1: "right", 2: "stop", 3:"u"}
	print("***************** Detected direction : "+map[int(preds1)])
	turn = map[int(preds1)]
	counter2 = time.time()
	os.rename('./test/test/camera_img.jpeg', './test/test/camera_img'+str(counter2)+'pred: '+turn+'.jpeg')
	source = './test/test/'
	dest1 = './images3/'
...
def threaded_obstacle_handling():
	...
	global turn, laser_running, counter
	############################################################################################
...
def laser_callback(msg):
	...
	global laser_running, deviation_running, counter
	# print(msg)
...
if __name__ == '__main__':
	try:
		...
		front_wall_thr = 0.45		## threshold for distance from wall at which bot should turn
		orientation = PI/2
		active_x = True
		active_y = False
		pos_x = 2.908154
